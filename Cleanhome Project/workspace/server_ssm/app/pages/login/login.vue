		...
		<view class="t-login">
			<form class="cl">
				<view class="t-a">
					...
					<input type="text" name="phone" placeholder="username reauired" maxlength="11" v-model="username" />
				</view>
				<view class="t-a">
					<image src="http://124.71.39.108/static/images/pwd.png"></image>
					<input type="password" name="code" maxlength="6" placeholder="password required" v-model="passwd" />
				</view>
				<button @tap="handLogin">Login</button>
				<view class="t-c">
					<text class="t-c-txt" @tap="toReg">register</text>
					<text>to home</text>
				</view>
			</form>
			<!--  #ifdef MP-WEIXIN -->
			<view class="t-f"><text>—————— 其他Login方式 ——————</text></view>
			<view class="t-e cl">
	...
	methods: {
		...mapActions(['updateUserInfo', 'logout']),
			...
			getUserInfos(e) {
				...
				uni.getUserProfile({
					provider: 'weixin',
					desc: 'Obtain user information Obtained header information nickname',
					lang: 'zh_CN',
					success: (e) => {
						console.log(e)
						this.wechatLogin()
			...
			handLogin() {
				...
				if (this.open_id) {
					console.log('------------------>handLogin:使用openidLogin:' + this.open_id)
					findj({
						params: {
							table: 'user',
							openid: this.open_id
					...
					console.log('------------------>handLogin:Username Password Login:')
...
.login-bg {
	height: 100vh;
	padding-top: 300rpx;
	background-image: url(http://124.71.39.108/static/images/bg3.png);
}
