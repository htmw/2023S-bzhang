...
function () {
  ...
  DomElement.prototype.removeClass = function (className) {
    ...
    return (0, _forEach["default"])(_context9 = this).call(_context9, function (elem) {
      ...
      var arr = elem.className.split(/\s/);
      arr = (0, _filter["default"])(arr).call(arr, function (item) {
        item = (0, _trim["default"])(item).call(item); // Delete class
...
function () {
  function DropList(menu, conf) {
    ...
    var $container = dom_core_1["default"]('<div class="w-e-droplist"></div>'); // Title
  ...
  if (option.beforeSend) {
    ...
    if (beforeResult && (0, _typeof2["default"])(beforeResult) === 'object') {
      if (beforeResult.prevent) {
        // 如果返回的结果是 {prevent: true, msg: 'xxxx'} 则表示User放弃上传
...
function () {
  ...
  SelectionAndRange.prototype.createRangeByElem = function ($elem, toStart, isContent) {
    ...
    if (isContent) {
      ...
      // 如果User没有传入 isContent 参数，那就默认为 false
...
function () {
  ...
  Text.prototype.html = function (val) {
    ...
    if (val == null) {
      ...
      html_1 = html_1.replace(const_1.EMPTY_P_LAST_REGEX, ''); // 为了避免User在最后生成的EMPTY_P标签中编辑数据, 最后产生多余标签, 去除所有p标签上的data-we-empty-p属性
       ...
       * 暂时注释, 等待进一步的兼容处理
       */
      // html = html.replace(/><br>(?!<)/gi, '>') // 过滤 <p><br>Content</p> 中的br
      // html = html.replace(/(?!>)<br></gi, '<') // 过滤 <p>Content<br></p> 中的br
  ...
  Text.prototype._saveRange = function () {
    ...
    function saveRange() {
      // 随时保存选区
      editor.selection.saveRange(); // 更新按钮 active Status
...
function pasteTextHtml(editor, pasteEvents) {
  function fn(e) {
    ...
    if (nodeName === 'CODE' || topNodeName === 'PRE') {
      ...
      return;
    } // 如果User开启闭粘贴样式注释则将复制进来为url的直接转为链接 否则不转换
    //  在群中有User提到关闭样式粘贴复制的文字进来后链接直接转为文字了，不符合预期，这里优化下
...
function () {
  ...
  Menus.prototype.init = function () {
    ...
    var _this = this; // 从User配置的 menus 入手，看需要初始化哪些菜单
...
function (_super) {
  ...
  function Head(editor) {
    ...
    var $elem = dom_core_1["default"]('<div class="w-e-menu" data-title="Title"><i class="w-e-icon-header"></i></div>');
    var dropListConf = {
      width: 100,
      title: '设置标题',
      type: 'list',
  ...
  Head.prototype.setMultilineHead = function (value) {
    ...
    var containerElem = (_a = $selection.getSelectionContainerElem()) === null || _a === void 0 ? void 0 : _a.elems[0]; // 白Name单：User选区里如果有该元素则不进行转换
    ...
    var endElem = dom_core_1["default"]($selection.getSelectionEndElem()); // 判断User选中元素是否为最后一个空元素，如果是将endElem指向上一个元素
...
function default_1(editor, text, link) {
  ...
  function checkLink(text, link) {
    ...
    if (check === undefined) {//User未能通过开发者的校验，且开发者不希望编辑器提示User
    } else if (check === true) {
      //User通过了开发者的校验
      return true;
    } else {
      //User未能通过开发者的校验，开发者希望我们提示这一字符串
    ...
    tabs: [{
      ...
      events: [// 插入链接
      {
        ...
        fn: function fn() {
          ...
          if (!text) text = link; // 校验链接是否满足User的规则，若不满足则不插入
  ...
  var tabsConf = [{
    ...
    events: [// 插入视频
    {
      ...
      fn: function fn() {
        ...
        if (!video) return; // 对当前User插入的内容进行判断，插入为空，或者返回false，都停止插入
...
function default_1(editor) {
   ...
   * @param containerClass 模板最外层的类Name
   * @param iconClass 模板中icon的类Name
   * @param titleName 模板中标题的Name 需要则设置不需要则设为空字符
   */
...
function (_super) {
  ...
  WrapListHandle.prototype.exec = function () {
    ...
    if (!$startElem.prior && !$endElem.prior || !($start === null || $start === void 0 ? void 0 : $start.prev().length) && !($end === null || $end === void 0 ? void 0 : $end.next().length)) {
      ...
      // 当 selectionElem 的标签Name和按钮类型 一致 的时候
      // 代表着当前的操作是 取消 序列
      // =====================================
      ...
      // 当 selectionElem 的标签Name和按钮类型 不一致 的时候
      // 代表着当前的操作是 转换 序列
      // =====================================
      else {
          // 创建 序列节点
    ...
    // =====================================
        ...
        // 当 selectionElem 的标签Name和按钮类型一致的时候
        // 代表着当前的操作是 取消 序列
        // =====================================
        ...
        // 当 selectionElem 的标签Name和按钮类型不一致的时候
        // 代表着当前的操作是 转换 序列
        // =====================================
        else {
            // 创建 文档片段
...
function (_super) {
  ...
  JoinListHandle.prototype.exec = function () {
    ...
    var $nodes = editor.selection.getSelectionRangeTopNodes(); // 获取开始段落和结束段落 标签Name
    ...
    if (startNodeName === endNodeName) {
      ...
      if ($nodes.length > 2) {
        ...
        // 由于开头序列 和 结尾序列的标签Name一样，所以只判断了开头序列的
        // 当开头序列的标签Name和按钮类型 一致 的时候
        // 代表着当前是一个 设置序列 的操作
        // =====================================
...
function (_super) {
  ...
  StartJoinListHandle.prototype.exec = function () {
    ...
    var $nodes = editor.selection.getSelectionRangeTopNodes(); // 获取开始段落标签Name
...
function (_super) {
  ...
  EndJoinListHandle.prototype.exec = function () {
    ...
    var $nodes = editor.selection.getSelectionRangeTopNodes(); // 获取结束段落标签Name
...
function preEnterListener(e, editor) {
  ...
  var $topElem = $selectionElem === null || $selectionElem === void 0 ? void 0 : $selectionElem.getNodeTop(editor); // 获取顶级节点节点Name
...
function createShowHideFn(editor) {
  ...
  function showSplitLineTooltip($splitLine) {
    ...
    var conf = [{
      $elem: dom_core_1["default"]("<span>" + editor.i18next.t('menus.panelMenus.Delete') + "</span>"),
...
function default_1(editor) {
  ...
  $textElem.attr('contenteditable', 'true').css('width', '100%').css('height', '100%'); // Add placeholder
  ...
  $textElem.addClass('w-e-text'); // Add ID
...
function _bindFocusAndBlur(editor) {
  ...
  function listener(e) {
    ...
    if (!isChild) {
      // 若为选择工具栏中的功能，则不视为成 blur Operate
...
function () {
  ...
  History.prototype.revoke = function () {
    ...
    this.editor.change.connect(); // 如果User在 onchange 中修改了内容（DOM），那么缓存中的节点数据可能不连贯了，不连贯的数据必将导致恢复失败，所以必须将User的 onchange 处于监控状态中
...
function compile(data) {
  ...
  (0, _forEach["default"])(data).call(data, function (record, index) {
    ...
    // 普通的数据补全：Title（纯文本内容）、加粗、斜体、删除线、下划线、颜色、背景色、字体、字号、列表（纯文本内容）
    // 特殊的数据补全：Title（包含 HTMLElement）、列表（包含 HTMLElement 或 ul -> ol 或 ol -> ul 或 Enter）
